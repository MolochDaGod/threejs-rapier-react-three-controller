...
export interface LocoBlendInput {
  ...
  active: boolean;
  /**
   * Playback rate for the (non-stride) idle tier. A small per-character offset
   * from 1 desyncs a crowd of standing characters so they don't all breathe in
   * lockstep ("blended time changes"). Defaults to 1. Stride clips ignore this —
   * their cadence stays phase-synced to `speed`.
   */
  idleRate?: number;
  /**
   * Initial phase (0..1 of the clip length) applied ONCE when an idle clip first
   * starts, so different characters begin their idle loop at different points.
   * Defaults to 0.
   */
  idlePhase?: number;
  dt: number;
  ...
  update(input: LocoBlendInput): void {
    ...
    const request = (id: string | undefined, weight: number, stride: boolean): void => {
      ...
      if (!entry) {
        ...
        // Offset a freshly-started idle to a per-character phase so a crowd of
        // standing characters isn't frozen at the same frame of the loop.
        if (!stride && input.idlePhase) {
          action.time = (input.idlePhase % 1) * entry.duration;
        }
    ...
    for (const [id, entry] of this.entries) {
      ...
      if (entry.stride) {
        ...
        entry.action.setEffectiveTimeScale(input.idleRate ?? 1);
      }
    }
  }
