...
/** Strand cross-section: 1/8 of one head pixel ("normal box"). */
export const STRAND_THICKNESS = P / 8;

/** Default hard cap on strand instances (one InstancedMesh draw call). */
export const MAX_STRANDS = 6000;
 ...
 * angles stay crisp) with a luminance-derived bump map for strand relief and
 * a sheen lobe that catches light the way real hair does. `seed` varies per
 * box so neighbouring boxes don't tile identically. The caller owns the
 * handle: `dispose()` frees the material and both textures.
 */
export function createHairBoxMaterial(box: ProtrusionBox, seed: number): HairBoxMaterial {
  // Side hair + beards get the FINE variant: 4×-thinner filaments packed
  // into thick clumps. Fine boxes double their texel density (still clamped)
  // so even small beard boxes have enough texels for individual filaments.
  ...
  const mat = new THREE.MeshPhysicalMaterial({
    ...
    bumpScale: light ? 0.006 : fine ? 0.02 : 0.012,
    roughness: light ? 0.5 : fine ? 0.55 : 0.62,
    metalness: 0,
    // Hair's signature specular: a soft sheen lobe tinted toward a lightened
    // version of the hair colour (fibre-scatter look, not plastic gloss).
    sheen: light || fine ? 1 : 0.85,
    sheenRoughness: light ? 0.35 : fine ? 0.38 : 0.45,
    sheenColor: new THREE.Color(shade(box.color, 1.35)),
    transparent: light,
    opacity: light ? 0.86 : 1,
  ...
  boxes.forEach((b, bi) => {
    ...
    for (let i = 0; i < cols; i++) {
      for (let j = 0; j < rows; j++) {
        ...
        const jx = (hash01(i, j, seed) - 0.5) * spacing * 0.8;
        const jz = (hash01(i, j, seed + 1) - 0.5) * spacing * 0.8;
        const len = Math.max(P, b.h * (0.75 + hash01(i, j, seed + 2) * 0.55));
        const hang = len > 0.25;
        out.push({
          ...
          len,
          thick: STRAND_THICKNESS * (1 + hash01(i, j, seed + 3) * 1.2),
          tiltX: (hash01(i, j, seed + 4) - 0.5) * 0.32,
          tiltZ: (hash01(i, j, seed + 5) - 0.5) * 0.32,
          color: shade(b.color, 0.82 + hash01(i, j, seed + 6) * 0.36),
          phase: hash01(i, j, seed + 7) * Math.PI * 2,
          // Long hanging strands sway in the wind; short scalp fuzz holds still.
          sway: hang ? Math.min(0.11, 0.03 + len * 0.09) : 0,
        });
      }
    }
  });
